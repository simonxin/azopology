...
Get-AzrgVizualization -ResourceGroups 'test-resource-group'
...
.NOTES
Project URL: https://github.com/simonxin/aztopologyviz
#>

function Get-AzrgVizualization {
    [alias("AzViz")]
    [CmdletBinding()]
    param (
        # Resource Groups 
    ...
    } | Export-PSGraph -ShowGraph:$ShowGraph -OutputFormat $OutputFormat -OutVariable Graph -DestinationPath $outputgraph
    #endregion graph-generation

    Write-Verbose "Graph Exported to path: $($Graph.fullname)"
}

Export-ModuleMember Get-AzNetworkVizualization -Alias aztopologyviz
